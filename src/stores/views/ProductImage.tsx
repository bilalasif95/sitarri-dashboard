...
export const ProductImage: React.FC<ProductImageProps> = ({
  ...
  const handleBack = () => navigate(storesUrl(productId));
  const handleUpdateSuccess = (data: ProductImageUpdate) => {
    if (data.storeImageupdate.storeErrors.length === 0) {
            ...
            {(updateImage, updateResult) => (
              <TypedProductImageDeleteMutation onCompleted={handleBack}>
                {(deleteImage, deleteResult) => {
                  ...
                  const image = data && data.store && data.store.mainImage;

                  return (
                    <>
                      <ProductImagePage
                        disabled={loading}
                        product={maybe(() => data.store.name)}
                        image={image || null}
